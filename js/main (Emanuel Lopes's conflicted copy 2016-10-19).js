...
(function() {
    ...
    function renderView(object) {
        //clean old data
        board.val('').removeClass('initial individual-conflict with-value finished').removeAttr('disabled');
    ...
    function newGame(level) {
    	...
    	timerGame = moment('00:00:00', 'HH:mm:ss');
    	timeoutGame = setInterval(timerGameInterval, 1000);
    }

    function timerGameInterval(){
    	timerGame.seconds(timerGame.seconds()+TIMER_INTERVAL);
    	console.log(timerGame.format('HH:mm:ss'));
    }

    function messageAlert(text) {
    	$('#time').text('Time: ' + timerGame.format('HH:mm:ss'));
    	$('#message').text(text);
    ...
    function check() {
    	...
    	$.ajax({
    		...
    		if(msg.conflicts.length === 0 && msg.finished){
